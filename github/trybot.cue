...
import (
	...
	"github.com/rogpeppe/cuetest-schemastore/src/schemas/json@v0"
)
...
workflows: trybot: #repo.bashWorkflow & {
	...
	on: {
		push: {
			branches: list.Concat([[#repo.testDefaultBranch], #repo.protectedBranchPatterns]) // do not run PR branches
	...
	jobs: {
		test: {
			...
			let _setupGoActionsCaches = #repo.setupGoActionsCaches & {
			...
			if: "\(#repo.containsTrybotTrailer) || ! \(#repo.containsDispatchTrailer)"
			...
			steps: [
				...
				json.#step & {
					if:  "\(#repo.isProtectedBranch) || \(_isLatestLinux)"
	...
	_testStrategy: {
		"fail-fast": false
		matrix: {
			"go-version": [#repo.previousStableGo, #repo.latestStableGo]
			runner: [#repo.linuxMachine, #repo.macosMachine, #repo.windowsMachine]
		}
	}
	...
	// work.
	_isLatestLinux: "(\(goVersion) == '\(#repo.latestStableGo)' && \(matrixRunner) == '\(#repo.linuxMachine)')"
	...
	_e2eTestSteps: [... json.#step & {
		...
		if: "github.repository == '\(#repo.githubRepositoryPath)' && \(#repo.isProtectedBranch) && \(_isLatestLinux)"
	}] & [
